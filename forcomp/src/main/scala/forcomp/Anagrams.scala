...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if (occurrences.isEmpty) List(List())
      ...
      count <- (0 to occurrences.head._2).toList
      combination <- combinations(occurrences.tail)
    } yield ((occurrences.head._1, count) +: combination).filter({
      case (_, c) => c > 0
    })
    ...
    def helper(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          restWords <- helper(subtract(occurrences, subset))
          if word.nonEmpty
