...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("mostRetweeted of empty set")
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(tweet => google.exists(kw => tweet.text.contains(kw)))
  lazy val appleTweets: TweetSet = allTweets.filter(tweet => apple.exists(kw => tweet.text.contains(kw)))
